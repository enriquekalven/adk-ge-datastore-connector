...
def test_category_b_and_c_adc_fallback():
    """Test 2: Verifies Category B & C Application Default Credentials (ADC) fallback when user OAuth token is omitted."""
    print("\n--- Test 2: Category B & C Application Default Credentials (ADC) Fallback ---")
    mock_context = MagicMock(spec=ToolContext)
    mock_context.state = {} # Empty state (no user OAuth token)
    ...
    with patch("tools.datastore_search._get_adc_token", return_value="Mock_ADC_Service_Account_Token_777"), \
        ...
        print("✅ Test 2 PASSED: Category B & C datastores fall back cleanly to ADC Service Account credentials without requiring user OAuth tokens!")

def test_tool_expired_token_handling():
    """Test 3: Verifies HTTP 401 token expiry handling."""
    print("\n--- Test 3: HTTP 401 Token Expiry Handling ---")
    mock_context = MagicMock(spec=ToolContext)
    ...
    with patch("requests.Session.post") as mock_post:
        ...
        print("✅ Test 3 PASSED: 401 Unauthorized caught and converted to AUTH_EXPIRED signal.")

def test_null_json_fields_handling():
    """Test 4: Verifies null/NoneType JSON fields extraction safety."""
    print("\n--- Test 4: Null JSON Fields Safety (derivedStructData: null) ---")
    mock_context = MagicMock(spec=ToolContext)
    ...
    with patch("requests.Session.post") as mock_post:
        ...
        print("✅ Test 4 PASSED: Null JSON fields handled safely without AttributeError.")

def test_input_sanitization():
    """Test 5: Verifies empty and oversized query sanitization."""
    print("\n--- Test 5: Input Query Sanitization ---")
    mock_context = MagicMock(spec=ToolContext)
    mock_context.state = {"enterprise_oauth": "Valid_Token"}
    
    empty_res = query_enterprise_datastore("   ", tool_context=mock_context)
    assert "valid, non-empty" in empty_res
    print("✅ Test 5 PASSED: Empty search query rejected cleanly.")

def test_rate_limiting_429_handling():
    """Test 6: Verifies HTTP 429 Rate Limit Handling."""
    print("\n--- Test 6: HTTP 429 Rate Limit Handling ---")
    mock_context = MagicMock(spec=ToolContext)
    mock_context.state = {"enterprise_oauth": "Valid_Token"}
    ...
    with patch("requests.Session.post") as mock_post:
        ...
        print("✅ Test 6 PASSED: 429 Rate limit caught and converted to friendly error.")
